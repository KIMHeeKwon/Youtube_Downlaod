...
### Phase 3 구현 — 사용법 투어 · 자동 업데이트
- **목표**: PHASE3 계획 §1~§6 구현 (릴리스 발행 §7은 사용자 확인 대기)
- **산출물**:
  - 신규 `tour.py`(145줄) — Step/Tour, 4분할 딤 패널 스포트라이트, 말풍선(진행 표시·건너뛰기)
  - 신규 `updater.py`(196줄) — version_tuple/parse_release/check_app_update/update_ytdlp/
    apply_app_update, 교체 스크립트 생성(백업·복원 포함)
  - `gui.py` 통합 — 최초 실행 투어 자동 표시, [사용법 보기] 버튼, 업데이트 배너(업데이트/나중에),
    설정 파일 `~/.youtube_downloader.json`, 시작 시 백그라운드 갱신 확인
  - 신규 `tests/test_updater.py`(12건), `README.md` 보강(보안 경고 상세·투어·업데이트 안내)
  - `implementation-notes.md` — 계획 이탈 3건 기록
- **검증 결과**:
  - 단위 테스트 31/31 통과 (updater 12건 신규 — 버전 자릿수 함정 0.9<0.10 포함)
  - `gui.py --selftest` 통과 (투어 생성/파괴 포함, 사용자 설정 미변경)
  - 투어 스포트라이트 기하 검증 4/4 단계 통과 — 스포트라이트 미차폐, 창 모서리 딤 적용,
    말풍선 비중첩·화면 내 (2단계 옵션 위젯 3개 합집합 420x39 정상)
- **현재 진행도**: 코드 완성. 화면 캡처 육안 확인은 터미널 화면 기록 권한 부재로 미수행
- **남은 미해결**: 업데이트 배너→교체→재시작 엔드투엔드 실증은 릴리스 미발행으로 불가
- **다음 단계**: 사용자 승인 후 v0.2.0 릴리스 발행 → 구버전 설치본에서 업데이트 실증
