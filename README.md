...
- ⚠️ **"Windows에서 PC를 보호했습니다"** 파란 창이 뜨면: **[추가 정보] → [실행]**
  - 바이러스 경고가 아니라 **배포자가 확인되지 않았다**는 안내입니다. 이 도구는 개인이
    만든 무료 도구라 유료 코드 서명 인증서를 쓰지 않으며, 설치 스크립트 내용은
    [windows/install.ps1](windows/install.ps1)에서 그대로 확인할 수 있습니다.
  - 백신이 차단하면 압축 푼 폴더를 예외로 등록한 뒤 다시 실행하세요.
...
**처음 실행하면 사용법 안내가 자동으로 뜹니다.** 화면 각 부분을 순서대로 짚어주는 4단계
안내이며, 나중에 다시 보려면 창 아래 **[사용법 보기]** 버튼을 누르세요.

### 업데이트 — 따로 받을 필요 없습니다

- **다운로드 엔진(yt-dlp)**: 실행할 때마다 자동으로 최신 버전이 됩니다. YouTube 내부 변경으로
  다운로드가 안 되던 문제가 대부분 이 갱신으로 해결됩니다.
- **프로그램 본체**: 새 버전이 나오면 실행 시 창 위에 알림 줄이 뜹니다.
  **[업데이트]**를 누르면 내려받기·교체·재시작까지 자동으로 진행됩니다.
  받아둔 영상(`downloads` 폴더)과 설정은 그대로 유지됩니다.
...
| 어느 날부터 다운로드 실패 | YouTube 내부 변경 — GUI를 껐다 켜면 yt-dlp가 자동 갱신됩니다. 터미널에서는 `uv sync --upgrade` |
| 업데이트 후 실행이 안 됨 | 프로젝트 폴더의 `update-error.log` 확인 — 자동 복원되므로 이전 버전으로 실행됩니다 |
...
| [docs/PHASE3.md](docs/PHASE3.md) | 사용법 투어·자동 업데이트 구현 계획 |
| [DECISIONS.md](DECISIONS.md) | 설계 결정 기록 (선택 근거와 기각한 대안) |
